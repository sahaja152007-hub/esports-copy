...
export const Route = createFileRoute("/teams/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [{ title: "Team not found — VEXA" }, { name: "robots", content: "noindex" }],
      };
    }
    const { team } = loaderData;
    const description = `${team.name} — ${team.region} · ${team.game}. Roster, record and upcoming Winter Cup matches.`;
    return {
      meta: [
        { title: `${team.name} — VEXA team profile` },
        { name: "description", content: description },
        { property: "og:title", content: `${team.name} — VEXA team profile` },
...
function TeamNotFound() {
  return (
    <main className="mx-auto max-w-7xl px-6 py-20 text-center">
      <h1 className="font-display text-2xl font-bold">That team isn't in the circuit</h1>
      <Link
        to="/teams"
  ...
  return (
    <main className="pb-16">
      ...
      <section className="mx-auto max-w-7xl px-6 py-4">
        <h2 className="font-display text-lg font-bold tracking-tight">Roster</h2>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <h2 className="font-display text-lg font-bold tracking-tight">Matches</h2>
